 ...
 * ファイルへプロパティを新規登録する。
 */
public class CreateProperty implements BiFunction<String, String, Integer> {
   ...
   */
  public CreateProperty(ContentsAttribute attr) {
    this.attr = attr;
  }
   ...
   * ファイルへプロパティを新規登録する。
   *
   * @param keyName 操作対象とするキーの名前を指定する。
  ...
  public Integer apply(String keyName, String keyValue) {
    ...
    try {
      final boolean result = service.createProperty(keyName, keyValue);
